...
{
                        // Kp, Ki, Kd, saturation
  m_roll_pid.set_all_params(0.5, 0, 0, (-ROLL_MAXIMUM_ANGULAR_SPEED)); //SIGN IS USED BECAUSE ROLL_MAXIMUM_ANGULAR_SPEED IS NEGATIVE, controller.h
	m_pitch_pid.set_all_params(0.5, 0, 0, (-PITCH_MAXIMUM_ANGULAR_SPEED)); //SIGN IS USED BECAUSE PITCH_MAXIMUM_ANGULAR_SPEED IS NEGATIVE, controller.h
	m_yaw_pid.set_all_params(0.5, 0, 0, (-YAW_MAXIMUM_ANGULAR_SPEED)); //SIGN IS USED BECAUSE YAW_MAXIMUM_ANGULAR_SPEED IS NEGATIVE, controller.h
...
void SpeedControl::setEnginesSpeed(float roll, float pitch, float yaw, float thrust){
  ...
  //engines in clockwise order, from top left
...
void SpeedControl::run() {
  ...
  // pids output
	...
	m_thrust_output = m_target_thrust;
  ...
  //thrust_file << "Target thrust speed: " << m_target_thrust << ", Current thrust speed: " << m_simulator->get_curr_thrust() << ", PID thrust output: "<< m_thrust_output << std::endl;

	//new angular speeds
	m_simulator->set_roll_speed(m_pid_roll_output);
	m_simulator->set_pitch_speed(m_pid_pitch_output);
	m_simulator->set_yaw_speed(m_pid_yaw_output);
	m_simulator->set_thrust(m_thrust_output);
